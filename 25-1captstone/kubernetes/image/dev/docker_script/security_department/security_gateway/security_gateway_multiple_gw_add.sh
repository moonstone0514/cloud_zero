...
ip route add ${EXT_SUBNET} dev ${EXT_DEV} src ${EXT_SRC} table ${TABLE_NAME} || true

echo ">> 기본 경로 설정: default via ${EXT_GW} dev ${EXT_DEV}"
ip route add default via ${EXT_GW} dev ${EXT_DEV} table ${TABLE_NAME} || true

# 내부 네트워크에 대한 정책 라우팅 설정
echo ">> 내부 네트워크 ${INT_SUBNET}에서 발생하는 트래픽은 ${TABLE_NAME} 테이블을 사용 (from rule)"
ip rule add from ${INT_SUBNET} table ${TABLE_NAME} || true

echo ">> 내부 네트워크 ${INT_SUBNET}으로 향하는 트래픽은 ${TABLE_NAME} 테이블을 사용 (to rule)"
ip rule add to ${INT_SUBNET} table ${TABLE_NAME} || true
